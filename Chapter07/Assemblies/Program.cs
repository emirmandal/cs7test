...
namespace Assemblies
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string hex = ReadLine();
            WriteLine($"Is a {hex} a valid color value: {hex.IsValidHex()}");

            Write("Enter a valid XML tag:");
            string xmlTag = ReadLine();
            WriteLine($"Is a {xmlTag} a valid XML tag: {xmlTag.IsValidXmlTag()}");

            Write("Enter a vaid password:");
            string password = ReadLine();
            WriteLine($"Is a {password} a valid password: {password.IsValidPassword()}");
